...
func main() {
	...
	for _, info := range files {
		...
		defer f.Close()
		img, _, _ := image.Decode(f) //从文件解出图片

		// Image has to be palleted before going into the GIF
		paletted := image.NewPaletted(img.Bounds(), palette.Plan9) // 新建调色板
		draw.FloydSteinberg.Draw(paletted, img.Bounds(), img, image.ZP)

		anim.Image = append(anim.Image, paletted)  // 添加图片
		anim.Delay = append(anim.Delay, delay*100) //每个图片的时间
	}

	f, _ := os.Create(output)
	defer f.Close()
	gif.EncodeAll(f, &anim) // 多张图片写入io.writer f
}
